...
;;<<< Copiaincollato senza pudore da language change
to create-network ; creazione link
  ...
  ;; make the first edge
  ask first-node [ create-link-with second-node [ set color blue ] ]
...
to update0
  ...
  if any? link-neighbors [
    ...
    let w (1 / var)
    let neigh-w (1 / [var] of random-neighbor )
    let new-mu (mu * w + neigh-w * [mu] of random-neighbor) / (w + neigh-w)
    set mu new-mu
    ;;set sigma sqrt((sigma * sigma + [sigma * sigma] of random-neighbor) / 2)
    set var 1 / (w + neigh-w)
...
to update1
  ...
  if any? link-neighbors [
    ...
    ;calcolo somma mu concordi-discordi e quanti sono
    foreach muneighs [
      mun ->
      ifelse abs( mun - mu ) <= (beta * sqrt(var0)) [
        set tot-conc (tot-conc + mun)
        set num-conc (num-conc + 1)
      ] [
        set tot-disc (tot-disc + mun)]
